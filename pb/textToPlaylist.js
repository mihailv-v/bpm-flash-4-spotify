...
async function searchForPlaylist(searchOption, searchTerm) {
    try {
          ...
          if (searchResults.length > 0) {
              ...
              searchResults.forEach((playlist, index) => {
                  ...
                  playlistLi.innerHTML = `
                      <input type="radio" id="search-result-${index}" name="search-result" value="${playlist.id}">
                      <label for="search-result-${index}">${playlist.name}</label>
                  `;
            ...
            let playlistSelected = null;
              while(!playlistSelected){
                await wait(getRandomArbitrary(0.2,2.625,'seconds')* 1000);
                  const selectedPlaylistRadioButton = document.querySelector('input[name="search-result"]:checked');
...
document.getElementById('search-playlist-button').addEventListener('click', async function () {
    ...
    try {
        const playlistId = await searchForPlaylist(selectedSearchOption, searchTerm);
        if (playlistId) {
...
document.getElementById('generateButton').addEventListener('click', async function() {
  async function generateRecommendations() {
      ...
      // An array of objects to store tracks from each run
      const runs = [];
      let playlistPart = 1;
      ...
      for (let i = 0; i < loopCount; i++) {
          ...
          if (i === 0) {
              // First run, get tracks from the original playlist
              tracks = await getPlaylistTracks(playlistName);
              shuffleArray(tracks);
          } else {
              // Subsequent runs, use tracks from previous recommendations
              const previousRun = runs[i - 1];
              tracks = previousRun.recommendations;
          }

          // Get recommendations for each track
          const recommendations = await getRecommendations(tracks,songsPerRecommendation,tempoOption,minTempo,maxTempo,targetTempo,minKey,maxKey,targetKey,selectedGenres);
      ...
      // Combine all tracks from all runs into a unique set
    ...
    if (recommendationArray.length > 0) {
        // Chunkify the recommendations into playlists if it exceeds 11,000 songs
        ...
        for (const chunk of recommendationChunks) {
            ...
            let playlistNameNew = `Recs of "${playlistName}" loop:${loopCount} `;
            if (playlistPart > 1) {
          ...
          let additionalRequirements = getAdditionalRecommendationRequirements(tempoOption,minTempo,maxTempo,targetTempo,minKey,maxKey,targetKey);
          if (additionalRequirements) {
              playlistNameNew += ` ${additionalRequirements.trim()}`;
          }

          
            console.log('Creating new playlist:', playlistNameNew.length>=200
                        ?(playslistNameNew.replaceAll("Tempo"," ").length>=200
                        ?playlistNameNew.replaceAll("Tempo"," ").substring(0,199)
                        :playslistNameNew.replaceAll("Tempo"," "))
                        :playlistNameNew);

            // Create the new playlist and get its ID
            const newPlaylistId = await createPlaylistWithName(playlistNameNew.length>=200
                                                              ?(playslistNameNew.replaceAll("Tempo"," ").length>=200
                                                              ?playlistNameNew.replaceAll("Tempo"," ").substring(0,199)
                                                              :playslistNameNew.replaceAll("Tempo"," "))
                                                              :playlistNameNew);
            ...
            // Add all unique tracks to the new playlist with the specified limit
            await addTracksToPlaylistRecommendation(accessToken, newPlaylistId, chunk);
            console.log('Adding unique tracks to the new playlist');


          // Display the results for the newly created playlist
          resultsDiv.innerHTML += `
              ...
              <p>New Playlist ID (Part ${playlistPart}): ${newPlaylistId}</p>
              <br>
          `;
            playlistPart++;
        }
